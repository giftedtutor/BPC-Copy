...
const MonthlyIncomeReport = () => {
  ...
  const getLedger = () => {
    ...
    if (currentMonth === '') {
      ...
      Axios.get(`${baseURL}/monthlyIncomeReport?month=${currentMonth}&&sort=asc&&colName=id&&pageNo=${1}&&records=999999999&&table=general_ledgers`)
        .then(response => {
          if (response.data.income.data[0] === undefined) {
            toast('No Monthly Income Report against provided Month!')
            setLoading2(false)
            setTotalDebit(0)
          } else {
  ...
  const filterName = suppliersF.filter(item => {
    if (item.name === null) {
      item.name = 'General Payment'
    }
    return filter !== "" ? item.name.toLowerCase().includes(filter.toLowerCase()) || item.actionDate.toLowerCase().includes(filter.toLowerCase()) || item.debit.toString().includes(filter.toString()) : item
  ...
  return (
    <div>
      <form>
        <h1 style={{ textAlign: 'center' }}>Monthly Income Report</h1>
