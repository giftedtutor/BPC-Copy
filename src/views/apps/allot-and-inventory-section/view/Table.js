...
const FilterData = () => {
  ...
  useEffect(() => {
    Axios.get(`${baseURL}/getAlotChildren?id=${id}&&pageNo=${pageNo}&&records=${record}&&sort=${sortType}&&colName=${column}`)
      .then(response => {
  ...
  const filterData = sorted.filter(item => {
    ...
    return filter !== "" ? item.quantity.toString().includes(filter.toString()) || item.status.toLowerCase().includes(filter.toLowerCase()) || item.name.toLowerCase().includes(filter.toLowerCase()) : item
  ...
  const FilteredData = filterData.map((data, index) => {
    return (
      <tr key={data.index}>
        <td>{data.name}</td>
        <td className='text-center'>{data.quantity.toLocaleString(undefined, { maximumFractionDigits: 2 })}</td>
        <td>{data.status}</td>
      </tr>
    )
